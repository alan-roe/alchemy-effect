...
> =>
  Match.value(config).pipe(
    ...
    Match.when({ method: "stored" }, () =>
      readCredentials<ProxmoxStoredCredentials>(
        profileName,
        "proxmox-stored",
      ).pipe(
...
const logout = (profileName: string, config: ProxmoxAuthConfig) =>
  Match.value(config).pipe(
    ...
    Match.when({ method: "stored" }, () =>
      deleteCredentials(profileName, "proxmox-stored").pipe(
        Effect.andThen(
...
const loginStored = Effect.fnUntraced(function* (profileName: string) {
  ...
  if (!skipTls) {
    const caInput = yield* Clank.text({
      message: "PEM-encoded CA cert (paste contents or leave empty to skip)",
    }).pipe(retryOnce);
    ca = caInput.trim().length > 0 ? caInput.trim() : undefined;
  }

  yield* writeCredentials<ProxmoxStoredCredentials>(
    profileName,
    "proxmox-stored",
    {
      type: "apiToken",
